# THIS CODE HAS BEEN DEPRECATED BECAUSE OF ITS POOR PERFORMANCE
...
def generate_service_request_wrapper(args):
    code, requisition_number, l4_code = args
    return generate_service_request(code, requisition_number, l4_code)
...
def generate_bundle(n):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=min(num_cpus, 8)) as executor:
        used_codes = []  # Initialize an empty list to store used codes
        service_requests = []
        for _ in range(n):
            ...
            while code in used_codes:
                code = f"code-{random.randint(1, 99999)}"
            # Save the code in the used_codes list
            used_codes.append(code)
            # Generate ServiceRequest in parallel
            future = executor.submit(generate_service_request_wrapper, (code, requisition_number, organization_l4["fullUrl"]))
            service_requests.append(future)
    
    # Append ServiceRequests to the bundle
    for service_request_future in service_requests:
        service_request = service_request_future.result()
        bundle["entry"].append({
...
if __name__ == "__main__":
    ...
    # Generate the bundle
    num_service_requests = 100000  # Change this to the desired number of ServiceRequests
